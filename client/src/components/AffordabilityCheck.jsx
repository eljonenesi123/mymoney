...
function AffordabilityCheck({ remainingBudget }) {
    ...
    <div className="card">
      ...
      {result && (
        <div className={styles.result} data-afford={result.after >= 0}>
          {result.after >= 0 ? (
            <p>
              Yes! You'd have <strong>{formatCurrency(result.after)}</strong> left this month.
            </p>
          ) : (
            <p>
              This would put you <strong>{formatCurrency(Math.abs(result.after))}</strong> over budget.
            </p>
          )}
        </div>
      )}
